...
def generate_html_file(json_file_path, template_path, output_file_path):
    """
    根据JSON文件和模板生成html文件
...
def process_year_month_folder(folder_path, template_path):
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.endswith('.json'):
            ...
            # 生成对应Markdown文件名规则：
            # 1. 当月总览文件（如2023-10.json -> 2023-10.html）
            # 2. 语言文件（如bing_zh-CN.json -> 2023-10_bing_zh-CN.html）
            if filename == f"{year_month}.json":
                html_file_name = f"{year_month}.html"
            elif filename.startswith('bing'):
                # 去除.json扩展名后拼接年月
                html_file_name = f"{year_month}_{filename[:-5]}.html"
            else:
                continue  # 跳过非标准命名文件
            ...
            generate_html_file(json_file_path, template_path, html_file_path)
...
def get_year_month_links(bing_dir, nums=10, current_folder=None):
    ...
    for i, folder in enumerate(year_month_folders, 1):
        # 检查目标html文件是否存在（如2023-10/2023-10.html）
        html_file = os.path.join(bing_dir, folder, f"{folder}.html")
        if os.path.exists(html_file):
            if current_folder:
                # 生成相对路径链接，设置起始路径为当前文件夹
                relative_path = os.path.relpath(html_file, start=os.path.join(bing_dir, current_folder))
            else:
                # 生成相对路径链接，设置起始路径为bing_dir
                relative_path = os.path.relpath(html_file, start=bing_dir)
            logging.info(f"Relative Path: {relative_path}")
            link = f'<a href="{relative_path}">{folder}</a>'
            current_row.append(f'<td>{link}</td>')
    ...
    # 补全最后未满nums个的单元格
    if current_row:
        while len(current_row) < nums:
            current_row.append('<td></td>')  # 空单元格占位
        row = f'<tr>{"".join(current_row)}</tr>'
        rows.append(row)

    # 构建完整的 HTML 表格
    html_table = f'<table>{"".join(rows)}</table>'
    return html_table


def generate_index_html(nums=10):
    """
